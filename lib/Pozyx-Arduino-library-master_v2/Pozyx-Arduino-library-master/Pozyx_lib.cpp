...
int PozyxClass::setInterruptMask(uint8_t mask, uint16_t remote_id)
{
  ...
  if(remote_id == NULL){
    ...
    wait_us(POZYX_DELAY_LOCAL_WRITE);
  }
  else{
    status = remoteRegWrite(remote_id, POZYX_INT_MASK, &mask, 1);
    wait_us(POZYX_DELAY_REMOTE_WRITE);
  }
...
int PozyxClass::setUWBSettings(UWB_settings_t *UWB_settings, uint16_t remote_id)
{
  ...
  if(remote_id == NULL){
    ...
    wait_us(2 * POZYX_DELAY_LOCAL_WRITE);    
  }else{
    status = remoteRegWrite(remote_id, POZYX_UWB_CHANNEL, tmp, 3);
    wait_us(2 * POZYX_DELAY_REMOTE_WRITE);
  }
...
char PozyxClass::getSystemError(uint16_t remote_id) //changement du type de variable en sortie plus un string mais un int 0=error (F avant)
{
  ...
  if(result != POZYX_SUCCESS)
    return faux;//("Error: could not connect with the Pozyx device");
  ...
  switch(error_code)
  {
    ...
    case POZYX_ERROR_I2C_WRITE:
      return faux; //("Error 0x01: Error writing to a register through the I2C bus");
    case POZYX_ERROR_I2C_CMDFULL:
      return faux ;//("Error 0x02: Pozyx cannot handle all the I2C commands at once");
    case POZYX_ERROR_ANCHOR_ADD:
      return faux ; //("Error 0x03: Cannot add anchor to the internal device list");
    case POZYX_ERROR_COMM_QUEUE_FULL:
      return faux ; //("Error 0x04: Communication queue is full, too many UWB messages");
    case POZYX_ERROR_I2C_READ:
      return faux ; //("Error 0x05: Error reading from a register from the I2C bus");
    case POZYX_ERROR_UWB_CONFIG:
      return faux ; //("Error 0x06: Cannot change the UWB configuration");
    case POZYX_ERROR_OPERATION_QUEUE_FULL:
      return faux ; //("Error 0x07: Pozyx cannot handle all the operations at once");
    case POZYX_ERROR_STARTUP_BUSFAULT:
      return faux ; //("Error 0x08: Internal bus error");
    case POZYX_ERROR_FLASH_INVALID:
      return faux ; //("Error 0x09: Flash memory is corrupted or invalid");
    case POZYX_ERROR_NOT_ENOUGH_ANCHORS:
      return faux ; //("Error 0x0A: Not enough anchors available for positioning");
    case POZYX_ERROR_DISCOVERY:
      return faux ; //("Error 0x0B: Error during the Discovery process");
    case POZYX_ERROR_CALIBRATION:
      return faux ; //("Error 0x0C: Error during the auto calibration process");
    case POZYX_ERROR_FUNC_PARAM:
      return faux ; //("Error 0x0D: Invalid function parameters for the register function");
    case POZYX_ERROR_ANCHOR_NOT_FOUND:
      return faux ; //("Error 0x0E: The coordinates of an anchor are not found");
    case POZYX_ERROR_FLASH:
      return faux ; //("Error 0x0F: Flash error");
    case POZYX_ERROR_MEMORY:
      return faux ; //("Error 0x10: Memory error");
    case POZYX_ERROR_RANGING:
      return faux ; //("Error 0x11: Ranging failed");
    case POZYX_ERROR_RTIMEOUT1:
      return faux ; //("Error 0x12: Ranging timeout");
    case POZYX_ERROR_RTIMEOUT2:
      return faux ; //("Error 0x13: Ranging timeout");
    case POZYX_ERROR_TXLATE:
      return faux ; //("Error 0x14: Tx was late");
    case POZYX_ERROR_UWB_BUSY:
      return faux ; //("Error 0x15: UWB is busy");
    case POZYX_ERROR_POSALG:
      return faux ; //("Error 0x16: Positioning failed");
    case POZYX_ERROR_NOACK:
      return faux ; //("Error 0x17: No Acknowledge received");
    case POZYX_ERROR_NEW_TASK:
      return faux ; //("Error 0xF1: Cannot create task");
    case POZYX_ERROR_UNRECDEV :
      return faux ; //("Error 0xFE: Hardware not recognized. Please contact support@pozyx.io");
    case POZYX_ERROR_GENERAL:
      return faux ; //("Error 0xFF: General error");
    default:
      return faux ; //("Unknown error");
  }
...
void __attribute__((weak)) __assert (const char *func, const char *file, int line, const char *failedexpr)
{
  ...
  if (Serial){
    pc.printf("Assertion in function : \n");
    pc.printf(func);
    pc.printf("Assertion failed : \n");
    pc.printf(failedexpr);
    pc.printf("Filename: \n");
    pc.printf(file);
    pc.printf("Line number: \n");
    pc.printf(line);
...
#else
/*id __attribute__((weak)) __assert_pozyx (const char *__func, const char *__file, int __lineno)
{
    // print out whatever you like here, function name, filename, line#, expression that failed.
  if (Serial){
    pc.printf("Assertion failed in function : \n");
    pc.printf(__func);
    pc.printf("Filename: \n");
    pc.printf(__file);
    pc.printf("Line number: \n");
