...
int PozyxClass::begin(bool print_result, int mode, int interrupts, int interrupt_pin){
  ...
  if(print_result){
    pc.printf("WhoAmI: 0x \n");
    //pc.printf(whoami, HEX);
    pc.printf("FW ver.: v ");
    //pc.printf((_fw_version&0xF0)>>4);
    pc.printf(".");
    //pc.printf((_fw_version&0x0F));
    if(_fw_version < 0x10)
      pc.printf(" (please upgrade)");
    pc.printf("\nHW ver.: ");
  ...
  if(print_result){
    pc.printf("selftest: 0b");
    //pc.printf(selftest, BIN);
  }
...
#else
    pc.printf("This is not a board supported by Pozyx, interrupts may not work \n");
    attachInterrupt(interrupt_pin, IRQ, RISING);
#endif
*/
...
int PozyxClass::sendTXBufferData(uint16_t destination)
{
  ...
  params[2] = 0x06;    
  status = regFunction(POZYX_TX_SEND, (uint8_t *)&params, 3, NULL, 0);
  wait(POZYX_DELAY_LOCAL_FUNCTION);
...
int PozyxClass::i2cWriteWrite(const uint8_t reg_address, const uint8_t *pData, int size)
{
  ...
  BUS.beginTransmission(POZYX_I2C_ADDRESS);
  // write the starting register address
  n = BUS.write(reg_address);        
  if (n != 1)
    return (POZYX_FAILURE);
    
  error = BUS.endTransmission(false); // hold the bus for a repeated start
  if (error != 0)
    return (POZYX_FAILURE);  

  BUS.beginTransmission(POZYX_I2C_ADDRESS);
  n = BUS.write(pData, size);  // write data bytes
  if (n != size)
    return (POZYX_FAILURE);

  error = BUS.endTransmission(true); // release the I2C-bus
  if (error != 0)
    return (POZYX_FAILURE);
...
int PozyxClass::i2cWriteRead(uint8_t* write_data, int write_len, uint8_t* read_data, int read_len)
{
  ...
  for(i=0; i<write_len; i++){
    n = BUS.write(*(write_data+i));  // write parameter bytes
  }

  if (n != 1)
    return (POZYX_FAILURE);
  
  n = BUS.endTransmission(false);    // hold the I2C-bus for a repeated start

  if (n != 0)
    return (POZYX_FAILURE);

  // Third parameter is true: relase I2C-bus after data is read.
  BUS.requestFrom(POZYX_I2C_ADDRESS, read_len, true);
  i = 0;
  
  while(BUS.available())
  {
    if(i<read_len)
      read_data[i++]=BUS.read();
    else  
      BUS.read();
  }
